...
class EventDump(sublime_plugin.EventListener):
    def on_pre_save(self, view):
        if not view.encoding() or view.encoding() == "Hexadecimal" or view.is_scratch() \
        or view.find("better_whitespace_ignore_all", 0) or view.settings().get('is_widget'):
            return
...
class EolWhitespaceCommand(sublime_plugin.TextCommand):
    def run(self, edit):
        ...
        for whitespace in whitespaces:
            if any([whitespace.intersects(self.view.line(sel)) for sel in self.view.sel()]) == \
            (self.view.settings().get("better_whitespace_area_of_effect", "unselected") == "unselected") == \
            (not self.view.settings().get("better_whitespace_area_of_effect", "selected") == "selected"):
                continue
